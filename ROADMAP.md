...
| # | Item | Why | Status |
|---|------|-----|--------|
| 14 | **Audit logging** — structured record of who changed what, when (`--verbose`/log file) | A tool that mutates org-wide security controls must be auditable | ⬜ |
...
| # | Item | Why | Status |
|---|------|-----|--------|
| 17 | Fix stale docstrings (e.g. `catalog_build` claims signed-raw-HTTP), accuracy-audit permissions docs, document single-region | Docs currently describe behavior the code doesn't have | ⬜ |
...
| 19 | Least-privilege IAM policy doc + threat model + `SECURITY.md` review | Table stakes for a security product going prod | ⬜ |
| 20 | Semver + CHANGELOG discipline + deprecation policy; validate against a live sandbox org before GA | Release readiness | ⬜ |
